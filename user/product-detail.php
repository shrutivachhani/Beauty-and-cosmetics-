...
if (isset($_POST['add_to_cart'])) {
    ...
    // Assume user ID is retrieved from the session
...
$email=$_SESSION['email'];
    if ($result->num_rows > 0) {
        ...
        $stmt = $con->prepare("INSERT INTO cart (user_id, name, image, price, description, product_id, quantity,email) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issssiis",$user_id, $name, $image, $price, $description, $product_id, $quantity,$email);
    }
    $stmt->execute();
