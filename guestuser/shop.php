        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo '
                        ...
                        <form method="POST" class="wishlist-icon-form">
                            <input type="hidden" name="product_id" value="' . htmlspecialchars($row['id']) . '">
                            <button type="button" 
                                    class="wishlist-icon ' . ($is_in_wishlist ? 'filled' : '') . '" 
                                    onclick="handleWishlistClick(' . ($user_id ? 'true' : 'false') . ', ' . htmlspecialchars($row['id']) . ')">
                                <img src="' . ($is_in_wishlist ? 'images/wishlist-filled-icon.svg' : 'images/wishlist-icon.svg') . '" 
                                     alt="Add to Wishlist" class="img-fluid">
                            </button>
                        </form>';
                ...
                if ($discount > 0) {
                    ...
                    echo '<span class="discounted-price">Rs. ' . number_format($original_price, 2) . '</span>';
                }
                ...
                if ($is_out_of_stock) {
                    ...
                    echo '<a href="product-detail.php?id=' . htmlspecialchars($row['id']) . '" class="btn btn-secondary mt-3">View Product</a>';
                }
...
<script>
    function handleWishlistClick(isLoggedIn, productId) {
        if (!isLoggedIn) {
            alert("Please login to add this item to your wishlist.");
        } else {
            alert("Please login to add this item to your wishlist.");
            // Add your AJAX request here if necessary to update the wishlist without redirecting.
